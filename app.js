...
function stepMonth(amount) { const index = months.indexOf(els.month.value) + amount; if (months[index]) { els.month.value = months[index]; render(); } }
...
async function init() {
  try {
    ...
    ["month","sensor","statistic","variable"].forEach(key => { const value = params.get(key); if ([...els[key].options].some(option => option.value === value)) els[key].value = value; });
    ...
    document.querySelector(".loader").remove(); els.empty.hidden = false; els.empty.querySelector("h3").textContent = "Figures could not be loaded"; els.empty.querySelector("p").textContent = "Generate figures.json, then reload this page."; els.stage.setAttribute("aria-busy", "false");
  }
}
